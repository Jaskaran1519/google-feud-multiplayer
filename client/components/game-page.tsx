...
export function GamePage() {
  ...
  const options = [
    "Pepperoni", "Cheese", "Mushrooms", "Onions", "Sausage",
    "Bacon", "Black olives", "Green peppers", "Pineapple", "Spinach"
  ]
  const players = [
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-purple-500 to-pink-500">
      <header className="bg-white/10 backdrop-blur-md p-4 text-white">
        <div className="container mx-auto flex justify-between items-center">
          <h1 className="text-2xl font-bold">Google Feud Multiplayer</h1>
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {options.map((option, index) => (
                  <Button key={index} variant="outline" className="h-12 text-left justify-start">
                    {index + 1}. {option}
                  </Button>
                ))}
              </div>
...
function PlayerList({ players }: { players: { id: number; name: string; score: number }[] }) {
  return (
...
function ChatArea({ messages }: { messages: { id: number; player: string; message: string }[] }) {
  return (
    <>
